...
static void fire_lead (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick, int te_impact, int hspread, int vspread, int mod)
{
	...
	tr = gi.trace(self->s.origin, NULL, NULL, cur_start, self, MASK_SHOT);
	...
	if (!(tr.fraction < 1.0))
	{
		...
		r = crandom()*hspread;
		u = crandom()*vspread;
		VectorMA(cur_start, 8192, forward, end);
		VectorMA(end, r, right, end);
		VectorMA(end, u, up, end);

		if (gi.pointcontents(cur_start) & MASK_WATER)
		{
			water = true;
			VectorCopy(cur_start, water_start);
			content_mask &= ~MASK_WATER;
		}

		while (true)
		{
			tr = gi.trace(cur_start, NULL, NULL, end, skip, content_mask);
			...
			if (tr.contents & MASK_WATER)
			{
				...
				if (!VectorCompare(cur_start, tr.endpos))
				{
					...
					VectorSubtract(end, cur_start, dir);
					vectoangles(dir, dir);
					AngleVectors(dir, forward, right, up);
					r = crandom()*hspread*2;
					u = crandom()*vspread*2;
				...
				tr = gi.trace(water_start, NULL, NULL, end, skip, MASK_SHOT);
			}
			else if (tr.ent && tr.ent->collision_model)
			{
				float dist;

				if (model_trace(tr.ent, cur_start, end, &dist, tr.plane.normal))
					VectorMA(cur_start, dist, aimdir, tr.endpos);
				else
				{
	...
	while (ignore)
	{
		...
		else
		{
			...
			if (tr.ent && tr.ent->collision_model)
			{
				...
				if (model_trace(tr.ent, from, end, &dist, tr.plane.normal))
